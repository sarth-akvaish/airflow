...
test.describe("Variables Page", () => {
  ...
  test("verify sorting when clicking column header", async () => {
    await variablesPage.clickColumnToSort("Key");
    const firstClickKeys = await variablesPage.getVariableKeys();

    expect(firstClickKeys.length).toBeGreaterThan(0);
    const sortedAsc = [...firstClickKeys].sort();
    const sortedDesc = [...firstClickKeys].sort().reverse();

    const isAscAfterFirstClick = JSON.stringify(firstClickKeys) == JSON.stringify(sortedAsc);
    const isDescAfterFirstClick = JSON.stringify(firstClickKeys) == JSON.stringify(sortedDesc);

    expect(isAscAfterFirstClick || isDescAfterFirstClick).toBeTruthy();
  });
  ...
  test("verify variables pagination", async () => {
    const limit = 3;

    await variablesPage.page.goto(`/variables?offset=0&limit=${limit}`);
    await variablesPage.waitForLoad();

    const page1Keys = await variablesPage.getVariableKeys();

    expect(page1Keys.length).toBeGreaterThan(0);

    await expect(variablesPage.paginationNextButton).toBeVisible({ timeout: 10_000 });
    await expect(variablesPage.paginationPrevButton).toBeVisible({ timeout: 10_000 });

    await variablesPage.clickNextPage();

    const page2Keys = await variablesPage.getVariableKeys();

    expect(page2Keys.length).toBeGreaterThan(0);
    expect(page2Keys).not.toEqual(page1Keys);

    await variablesPage.clickPrevPage();

    const backKeys = await variablesPage.getVariableKeys();

    expect(backKeys.length).toBeGreaterThan(0);
  });
